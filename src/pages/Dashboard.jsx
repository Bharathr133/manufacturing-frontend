...
export default function Dashboard() {
    ...
    // Safely unwrap API responses — handles { data: val }, val directly, or { data: { count: val } }
    ...
    const downloadReport = () => {
        const blob = new Blob([JSON.stringify({
            ...
            stats: { totalMachines: stats.machineCount, runningMachines: stats.runningCount, totalOrders: stats.orderCount, quality: { totalChecks: stats.totalChecks, passed: stats.passedChecks, rework: stats.reworkChecks, failed: stats.failedChecks, defectRate: `${stats.defectRate.toFixed(1)}%` } },
            recentQualityChecks: stats.recentChecks
        }, null, 2)], { type: "application/json" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url; a.download = `manufacturing-report-${new Date().toISOString().split("T")[0]}.json`; a.click();
        URL.revokeObjectURL(url);
        showToastMsg("Report downloaded successfully!");
    };

    const pct = (a, b) => b > 0 ? ((a / b) * 100).toFixed(0) : 0;
    const uptimePct = pct(stats.runningCount, stats.machineCount);
    ...
    const defectColors = getDefectColor(stats.defectRate);
            ...
            <div style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif", maxWidth: 1280, margin: "0 auto", padding: "0 4px" }}>
                ...
                {loading ? <SkeletonRow count={2} /> : (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16, marginBottom: 24 }}>
                        ...
                        <StatCard
                            label="Running Now"
                            value={services.machines === SERVICE_STATUS.OFFLINE ? "—" : stats.runningCount}
                            ...
                            offlineMsg="Machine service offline"
                        />
                        {services.machines === SERVICE_STATUS.ONLINE && stats.machineCount > 0 && (
                            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: "18px 20px", display: "flex", flexDirection: "column", gap: 8, gridColumn: "span 2" }}>
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
                                    <span style={{ fontSize: 12, color: "#94a3b8" }}>{stats.runningCount} running</span>
                                    <span style={{ fontSize: 12, color: "#94a3b8" }}>{stats.machineCount - stats.runningCount} idle / off</span>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        {loading ? <SkeletonRow count={1} /> : (
                            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16, marginBottom: 24 }}>
                                <StatCard
                                    ...
                                    value={services.production === SERVICE_STATUS.OFFLINE ? "—" : stats.orderCount}
                ...
                {isManager && (
                    <>
                        ...
                        {loading ? <SkeletonRow count={4} /> : services.quality === SERVICE_STATUS.OFFLINE ? (
                            <OfflineAlert service="Quality" />
                        ) : (
                            <>
                                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 16 }}>
                                    {/* Defect Rate — prominent card */}
                                    <div style={{ background: defectColors.bg, border: `1.5px solid ${defectColors.badge}`, borderRadius: 14, padding: "18px 20px", display: "flex", flexDirection: "column", gap: 8 }}>
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                            ...
                                            {stats.defectRate < 5 ? <ShieldCheck size={20} color={defectColors.text} /> : stats.defectRate < 15 ? <AlertTriangle size={20} color={defectColors.text} /> : <XCircle size={20} color={defectColors.text} />}
                                        </div>
                                        <span style={{ fontSize: 36, fontWeight: 800, color: defectColors.text, letterSpacing: "-1px", lineHeight: 1 }}>{stats.defectRate.toFixed(1)}%</span>
                                        <span style={{ fontSize: 12, background: defectColors.badge, color: defectColors.badgeText, padding: "3px 10px", borderRadius: 20, alignSelf: "flex-start", fontWeight: 600 }}>
                                            {stats.defectRate < 5 ? "Excellent quality" : stats.defectRate < 15 ? "Needs improvement" : "Critical attention"}
                                        </span>
                                    </div>

                                    <StatCard label="Total Checks" value={stats.totalChecks} icon={<TrendingUp size={20} color="#6366f1" />} accent="#6366f1" accentBg="#eef2ff" sub={`${stats.passedChecks} passed, ${stats.failedChecks} failed`} onClick={() => navigate("/quality")} />
                                    <StatCard label="Passed" value={stats.passedChecks} icon={<CheckCircle size={20} color="#10b981" />} accent="#10b981" accentBg="#f0fdf4" sub={`${pct(stats.passedChecks, stats.totalChecks)}% of checks`} onClick={() => navigate("/quality")} />
                                    <StatCard label="Rework Needed" value={stats.reworkChecks} icon={<AlertTriangle size={20} color="#f59e0b" />} accent="#f59e0b" accentBg="#fffbeb" sub={`${pct(stats.reworkChecks, stats.totalChecks)}% of checks`} onClick={() => navigate("/quality")} />
                                    <StatCard label="Failed" value={stats.failedChecks} icon={<XCircle size={20} color="#ef4444" />} accent="#ef4444" accentBg="#fef2f2" sub={`${pct(stats.failedChecks, stats.totalChecks)}% of checks`} onClick={() => navigate("/quality")} />
                                </div>

                                {/* Quality Breakdown Bar */}
                                {stats.totalChecks > 0 && (
                                    <QualityBar passed={stats.passedChecks} rework={stats.reworkChecks} failed={stats.failedChecks} total={stats.totalChecks} />
                                )}
                    ...
                    <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: 24, marginTop: 24 }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                            <div style={{ border: "1px solid #f1f5f9", borderRadius: 10, padding: 16, background: "#f8fafc" }}>
                                ...
                                <ReportRow label="Total Orders" value={services.production === SERVICE_STATUS.OFFLINE ? "Offline" : stats.orderCount} offline={services.production === SERVICE_STATUS.OFFLINE} />
                                <ReportRow label="Running Machines" value={services.machines === SERVICE_STATUS.OFFLINE ? "Offline" : stats.runningCount} offline={services.machines === SERVICE_STATUS.OFFLINE} />
                            </div>
                            <div style={{ border: "1px solid #f1f5f9", borderRadius: 10, padding: 16, background: "#f8fafc" }}>
                                <p style={{ margin: "0 0 12px", fontSize: 13, fontWeight: 700, color: "#334155" }}>Quality Summary</p>
                                <ReportRow label="Total Checks" value={services.quality === SERVICE_STATUS.OFFLINE ? "Offline" : stats.totalChecks} offline={services.quality === SERVICE_STATUS.OFFLINE} />
                                <ReportRow label="Defect Rate" value={services.quality === SERVICE_STATUS.OFFLINE ? "Offline" : `${stats.defectRate.toFixed(1)}%`} color={defectColors.text} offline={services.quality === SERVICE_STATUS.OFFLINE} />
                            </div>
                        </div>
                    </div>
                )}
...
function StatCard({ label, value, icon, accent, accentBg, sub, onClick, offline, offlineMsg }) {
    return (
        <div
            onClick={onClick}
            style={{ background: offline ? "#f8fafc" : "#fff", border: `1px solid ${offline ? "#e2e8f0" : "#e2e8f0"}`, borderRadius: 14, padding: "18px 20px", cursor: onClick ? "pointer" : "default", transition: "box-shadow 0.15s, transform 0.15s", position: "relative", animation: "fadeUp 0.3s ease both" }}
    ...
    return (
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: "16px 20px" }}>
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 10 }}>
                <LegendDot color="#10b981" label={`Passed ${pw}%`} />
                <LegendDot color="#f59e0b" label={`Rework ${rw}%`} />
                <LegendDot color="#ef4444" label={`Failed ${fw}%`} />
            </div>
        </div>
    );
}
                ...
                return (
                    <div key={key} style={{ display: "flex", alignItems: "center", gap: 6, background: isLoading ? "#f8fafc" : isOnline ? "#f0fdf4" : "#fef2f2", border: `1px solid ${isLoading ? "#e2e8f0" : isOnline ? "#bbf7d0" : "#fecaca"}`, borderRadius: 20, padding: "4px 12px 4px 10px", fontSize: 12, fontWeight: 600, color: isLoading ? "#94a3b8" : isOnline ? "#166534" : "#991b1b" }}>
                        {isLoading ? (
                            ...
                            <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#22c55e", boxShadow: "0 0 0 2px #bbf7d0" }} />
                        ) : (
...
function OfflineAlert({ service }) {
    return (
        <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 12, padding: "14px 18px", marginBottom: 24, display: "flex", alignItems: "center", gap: 10 }}>
            <WifiOff size={18} color="#ef4444" />
            <div>
                <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: "#991b1b" }}>{service} service is currently offline</p>
                <p style={{ margin: "2px 0 0", fontSize: 12, color: "#f87171" }}>Data will appear here when the service reconnects. Try refreshing.</p>
            </div>
        </div>
    );
}
...
function SkeletonRow({ count }) {
    return (
        <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(count, 4)}, 1fr)`, gap: 16, marginBottom: 24 }}>
            {Array.from({ length: count }).map((_, i) => (
                <div key={i} style={{ background: "linear-gradient(90deg, #f1f5f9 25%, #e2e8f0 50%, #f1f5f9 75%)", backgroundSize: "200% 100%", animation: "shimmer 1.5s infinite", borderRadius: 14, height: 110 }} />
            ))}
